...
_基于 MVP PRD、桌面端可交互原型与产品导览设计形成的独立说明文档；当前原型中的行情、公司资料与回答均为模拟数据。_
...
- 持仓与自选的标的切换；
- 快捷追问、自由输入和分析展开；
- 证据面板、判断编辑与影响复核；当前“研究档案”入口用于表达汇总核验方向，其完整聚合内容仍待实现；
- 买卖问题的安全回答；
- 5 步首次使用导览及键盘操作。

当前原型不能证明：真实行情与公告覆盖率、模型事实准确率、复杂事件归因能力、通知长期效果、用户真实持仓授权意愿或任何投资收益。它使用本地模拟数据，不连接真实账户、行情或模型服务。

桌面端是当前已实现范围。移动端应保留同一条对话主线，只将右侧详情面板改为底部 Sheet，将标的切换器改为适合窄屏的选择器；这一适配尚未由当前 HTML 原型验证。

相关材料：

- [MVP 交互方案与 PRD](./AI投研助手MVP交互方案与PRD.md)
- [桌面端原型设计](./docs/superpowers/specs/2026-09-09-ai-investment-assistant-desktop-prototype-design.md)
- [产品导览设计](./docs/superpowers/specs/2026-09-09-ai-investment-assistant-product-tour-design.md)
- [桌面端可交互原型](./ai-investment-assistant-desktop.html)
